...
function dCell(text: string, shade: boolean): TableCell {
  return new TableCell({
    ...
    children: [
      new Paragraph({ children: [new TextRun({ text: String(text ?? ''), size: 18 })] }),
    ],
  });
}

function aaslHeaderParagraphs(title: string, filter: Record<string, any>): Paragraph[] {
  ...
  if (deptName) {
    divisionRows.push(new Paragraph({
      children: [
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          ...
          bold: true, color: DARK_NAVY, size: 28,
        }),
      ],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: 'BANDARANAIKE INTERNATIONAL AIRPORT', size: 22 })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
...
function footerParagraph(): Paragraph {
  ...
  return new Paragraph({
    children: [new TextRun({ text: `PRINT DATE/TIME : ${ts}`, size: 16, italics: true })],
  });
}

// ── legacy (kept for backward compat) ────────────────────────────────────────
export async function generateWord(records: AttendanceReportRawData[]): Promise<Buffer> {
...
export async function generateFormattedReportWord(
  ...
  if (reportType === 'serial') {
    ...
    const headerRow = new TableRow({ children: ['SERIAL NO', 'EPF NO', 'NAME'].map(hCell) });
    ...
    tableNode = new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: [headerRow, ...dataRows] });
    ...
    if (filter.fromDate) {
      extraParagraphs = [
        new Paragraph({
          children: [new TextRun({ text: `FROM : ${filter.fromDate}  TO : ${filter.toDate ?? ''}`, size: 20 })],
        }),
        new Paragraph({ children: [] }),
      ];
    }
    const headerRow = new TableRow({ children: ['#', 'EPF NO', 'NAME'].map(hCell) });
    const dataRows = records.map((r, i) =>
      new TableRow({
        children: [
    ...
    if (filter.fromDate) {
      extraParagraphs = [
        new Paragraph({
          children: [new TextRun({ text: `MONTH : ${filter.fromDate?.slice(0, 7) ?? ''}`, size: 20 })],
        }),
        new Paragraph({ children: [] }),
      ];
    }
    const headerRow = new TableRow({ children: ['EPF NO', 'NAME', 'MONTH', 'DAYS PRESENT'].map(hCell) });
    const dataRows = records.map((r, i) =>
      new TableRow({
        children: [
          dCell(String(r.badgeNumber ?? ''),  i % 2 === 0),
    ...
    const headerRow = new TableRow({ children: ['EPF NO', 'NAME', 'DATE', 'TIME', 'TYPE'].map(hCell) });
    const dataRows = records.map((r, i) => {
      ...
      return new TableRow({
        children: [
          ...
          dCell(String(r.name ?? ''),                    i % 2 === 0),
          dCell(isNaN(d.getTime()) ? '-' : d.toLocaleDateString('en-GB'), i % 2 === 0),
          dCell(isNaN(d.getTime()) ? '-' : d.toLocaleTimeString('en-GB'), i % 2 === 0),
          dCell(r.checkTypeDisplay ?? (r.checkType === 'I' ? 'IN' : 'OUT'), i % 2 === 0),
        ],
      });
    });
    tableNode = new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: [headerRow, ...dataRows] });
